    ...
    for (const farmer of input.farmers) {
        ...
        for (const state of states) {
            for (const placement of placementCandidates(state.loads, capacities, farmer, Boolean(input.trailer))) {
                ...
                next.push(placed);
                const transferAllowedHere = onlyTransferAfterOrder === undefined || farmer.order === onlyTransferAfterOrder;
        ...
        states = dedupeAndRank(next, capacities, beamWidth);
    }
...
function dedupeAndRank(states, capacities, limit) {
    const best = new Map();
    for (const state of states) {
    ...
    const ranked = [...best.values()].sort((a, b) => compareStates(a, b, capacities));
    ...
    return selected.sort((a, b) => compareStates(a, b, capacities));
}
