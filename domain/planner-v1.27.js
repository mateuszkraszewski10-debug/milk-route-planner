...
function placementCandidates(loads, capacities, farmer, hasTrailer, remainingFarmers = []) {
    ...
    // For a large farm, also build strategic variants that reserve one whole
    // compartment for a later smaller farm. This allows the large farm to top up
    // a partly filled truck compartment instead of forcing a later small farm to split.
    if (minimumK !== undefined && remainingFarmers.length > 0) {
        for (const future of remainingFarmers.slice(0, 6)) {
            const futureAccessible = future.trailerAccess && hasTrailer ? [0, 1, 2, 3, 4, 5] : [0, 1, 2];
            if (!futureAccessible.some((i) => capacities[i] >= future.forecastLiters))
                continue;
            for (const reserve of futureAccessible) {
                if (!accessible.includes(reserve))
                    continue;
                const reserveFree = capacities[reserve] - loads[reserve];
                if (reserveFree < future.forecastLiters)
                    continue;
                const adjusted = free.map((x) => ({
                    i: x.i,
                    free: x.i === reserve ? x.free - future.forecastLiters : x.free,
                })).filter((x) => x.free > 0);
                for (let k = minimumK; k <= Math.min(minimumK + 1, adjusted.length); k++) {
                    const combos = combinations(adjusted, k).filter((combo) => combo.reduce((sum, x) => sum + x.free, 0) >= volume);
                    out.push(...placementsFromCombos(combos, volume));
                }
            }
        }
