...
function searchRoute(input, onlyTransferAfterOrder) {
    ...
    let states = [{ loads: [0, 0, 0, 0, 0, 0], rows: [], directTrailerLiters: 0, splitExtra: 0, tightPlacementPenalty: 0 }];
...
function placementCandidates(loads, capacities, farmer, hasTrailer, remainingFarmers = []) {
    ...
    // If the whole pickup fits in one currently available compartment, it must stay together.
    const singles = free.filter((x) => x.free >= volume).map((x) => [{ compartment: x.i, liters: volume }]);
    if (singles.length > 0)
        return singles;

    // A pickup that would fit in one empty compartment may never be split just
    // because earlier choices used the available space badly.
...
function dedupeAndRank(states, capacities, limit, remainingFarmers = [], hasTrailer = false) {
    ...
    return selected.sort(compareWithFuture);
}
