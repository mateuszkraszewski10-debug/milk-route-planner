...
const LAST_PROFILE_KEYS = {
    truck: 'milk_route_planner_last_truck',
    trailer: 'milk_route_planner_last_trailer',
};
function savedProfiles(kind) {
    return store.loadAll().filter((p) => p.kind === kind);
}
function selectedProfileRegistration(kind) {
    const profiles = savedProfiles(kind);
    if (profiles.length === 0)
        return '';
    const stored = localStorage.getItem(LAST_PROFILE_KEYS[kind]) ?? '';
    if (profiles.some((p) => p.registration === stored))
        return stored;
    return profiles[profiles.length - 1].registration;
}
function rememberSelectedProfile(kind, registration) {
    if (registration)
        localStorage.setItem(LAST_PROFILE_KEYS[kind], registration);
    else
        localStorage.removeItem(LAST_PROFILE_KEYS[kind]);
}
function profileOptions(kind) {
    const selected = selectedProfileRegistration(kind);
    return savedProfiles(kind)
        .map((p) => `<option value="${p.registration}" ${p.registration === selected ? 'selected' : ''}>${p.registration} · ${p.capacitiesLiters.join(' / ')} l</option>`)
        .join('');
}
...
function render() {
    app.innerHTML = `
  <header class="hero"><div><p class="eyebrow">Milk Route Planner <span class="version-badge">v1.36</span></p><h1>Planowanie komór mleczarki</h1><p>Cały plan przed wyjazdem · komory 1–6 · minimalna konieczna liczba przepompowań</p><div id="connection-status" class="connection-status">Sprawdzam połączenie…</div></div><div class="reserve-badge">65 l<br><small>stałego zapasu / komorę</small></div></header>
      ...
      <div class="saved-grid">
        <div class="saved-profile-control">
          ...
          <div class="profile-actions">
            <button type="button" class="secondary" data-edit-profile="truck">Edytuj</button>
            <button type="button" class="secondary" data-new-profile="truck">Dodaj nowe auto</button>
        ...
        <div class="saved-profile-control">
          ...
          <div class="profile-actions">
            <button type="button" class="secondary" data-edit-profile="trailer">Edytuj</button>
            <button type="button" class="secondary" data-new-profile="trailer">Dodaj nową przyczepę</button>
            <button type="button" class="delete-profile" data-delete-profile="trailer" disabled>Usuń zapisaną przyczepę</button>
          </div>
        </div>
      </div>
      <div class="vehicle-grid">
        <div id="truck-editor-wrap" class="vehicle-editor-wrap">${vehicleFields('truck', 'Auto · komory 1–3', [5300, 5300, 5300])}</div>
        <div id="trailer-wrap"><div id="trailer-editor-wrap" class="vehicle-editor-wrap">${vehicleFields('trailer', 'Przyczepa · komory 4–6', [5100, 5000, 6500])}</div></div>
      </div>
    ...
    <div class="footer-shell">
      <div class="footer-brand">
        <div class="footer-logo-wrap" aria-hidden="true">
          <img src="./icon.svg" alt="" class="footer-logo">
          <span class="footer-logo-glow"></span>
        </div>
        <div class="footer-brand-copy">
          <span class="footer-kicker">Milk Route Planner</span>
          <strong>Projekt i wykonanie: Mateusz Kraszewski</strong>
          <small>© 2026 Mateusz Kraszewski · Wszelkie prawa zastrzeżone.</small>
        </div>
      </div>

      <div class="footer-safety">
        <div class="footer-warning-icon" aria-hidden="true">!</div>
        <div>
          <h3>Ważne — odpowiedzialność kierowcy</h3>
          <p><strong>Milk Route Planner jest wyłącznie narzędziem pomocniczym.</strong> Wynik kalkulatora nie zastępuje wiedzy kierowcy, obowiązujących procedur, instrukcji przewoźnika ani kontroli rzeczywistego stanu pojazdu i surowca.</p>
          <p>Przed rozpoczęciem i w trakcie trasy kierowca powinien samodzielnie sprawdzić poprawność planu, w szczególności pojemności komór, rzeczywistą ilość mleka, wymagany zapas, kolejność odbiorów oraz możliwość i poprawność przepompowań. <strong>Korzystający z aplikacji pozostaje odpowiedzialny za decyzje podjęte na podstawie jej wskazań oraz za prawidłowy załadunek i stan przewożonego surowca.</strong></p>
          <p class="footer-legal">W zakresie dopuszczalnym przez obowiązujące prawo autor aplikacji nie ponosi odpowiedzialności za szkody, straty, przekroczenia pojemności, pogorszenie jakości surowca ani inne skutki wynikające z błędnych danych wejściowych, błędu obliczeniowego lub polegania wyłącznie na wyniku aplikacji bez jego weryfikacji.</p>
          <div class="footer-checks" aria-label="Lista kontroli przed użyciem planu">
            <span>✓ sprawdź litry</span>
            <span>✓ sprawdź pojemności</span>
            <span>✓ sprawdź zapas</span>
            <span>✓ sprawdź przepompowania</span>
          </div>
        </div>
      </div>
    </div>
...
function syncVehicleEditor(kind, forceOpen = false) {
    const profiles = savedProfiles(kind);
    const select = document.querySelector(`#saved-${kind}`);
    const selectedReg = select?.value || selectedProfileRegistration(kind);
    const selected = profiles.find((p) => p.registration === selectedReg);
    if (select && selectedReg)
        select.value = selectedReg;
    if (selected)
        fillProfile(kind, selected);
    const wrap = document.querySelector(`#${kind}-editor-wrap`);
    if (wrap)
        wrap.hidden = Boolean(selected) && !forceOpen;
    const editButton = document.querySelector(`[data-edit-profile="${kind}"]`);
    if (editButton)
        editButton.hidden = !selected;
}
function syncAllVehicleEditors() {
    syncVehicleEditor('truck');
    syncVehicleEditor('trailer');
}
...
function calculatePlanInWorker(input) {
    return new Promise((resolve, reject) => {
        const worker = new Worker(new URL('./planner-worker-v1.33.js', import.meta.url), { type: 'module' });
        ...
        worker.addEventListener('message', (event) => {
            ...
            if (event.data?.ok)
                ...
                reject(new Error(event.data?.error || 'Nie udało się policzyć planu.'));
        }, { once: true });
        worker.addEventListener('error', (event) => {
            cleanup();
            reject(new Error(event.message || 'Błąd modułu planera.'));
        }, { once: true });
...
function bindEvents() {
    ...
    document.querySelector('#saved-truck')?.addEventListener('change', (event) => {
        ...
        if (profile) {
            rememberSelectedProfile('truck', reg);
    ...
    document.querySelector('#saved-trailer')?.addEventListener('change', (event) => {
        ...
        if (profile) {
            rememberSelectedProfile('trailer', reg);
    ...
    document.querySelectorAll('[data-edit-profile]').forEach((button) => button.addEventListener('click', () => {
        const kind = button.dataset.editProfile;
        syncVehicleEditor(kind, true);
        document.querySelector(`#${kind}-registration`)?.focus();
    }));
    document.querySelectorAll('[data-new-profile]').forEach((button) => button.addEventListener('click', () => {
        const kind = button.dataset.newProfile;
        const select = document.querySelector(`#saved-${kind}`);
        if (select)
            select.value = '';
        const reg = document.querySelector(`#${kind}-registration`);
        if (reg)
            reg.value = '';
        const defaults = kind === 'truck' ? [5300, 5300, 5300] : [5100, 5000, 6500];
        defaults.forEach((value, i) => {
            const input = document.querySelector(`#${kind}-c${i}`);
            if (input)
    ...
    document.querySelectorAll('[data-delete-profile]').forEach((button) => button.addEventListener('click', () => {
        ...
        rememberSelectedProfile(kind, '');
        if (select) {
            select.innerHTML = `<option value="">— wybierz —</option>${profileOptions(kind)}`;
            const nextReg = selectedProfileRegistration(kind);
            select.value = nextReg;
            if (nextReg)
                rememberSelectedProfile(kind, nextReg);
        }
        syncVehicleEditor(kind);
        syncDeleteProfileButtons();
    }));
    document.querySelectorAll('[data-save-profile]').forEach((button) => button.addEventListener('click', () => {
        ...
        try {
            const profile = readProfile(prefix);
            store.save(profile);
            rememberSelectedProfile(prefix, profile.registration);
            render();
        }
        catch (error) {
    ...
    document.querySelector('#calculate')?.addEventListener('click', async (event) => {
        animateActionButton(event.currentTarget, 'calculate-pulse', 650);
        const output = document.querySelector('#plan-output');
        if (!output)
            return;
        const calculateButton = event.currentTarget;
        const originalLabel = calculateButton.textContent;
        calculateButton.disabled = true;
        calculateButton.textContent = 'Liczenie…';
        output.innerHTML = '<div class="precalc-summary"><strong>Liczenie planu…</strong> Możesz dalej przewijać stronę — aplikacja nie powinna się zawiesić.</div>';
        try {
            ...
            const plan = await calculatePlanInWorker({ truck, trailer, farmers, preferredTransferAfterFarmerOrder });
    ...
    for (const kind of ['truck', 'trailer']) {
        ...
        if (select && selected) {
            ...
            rememberSelectedProfile(kind, selected);
        }
    }
    syncAllVehicleEditors();
    syncFarmerListVisibility();
